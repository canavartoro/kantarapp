...
namespace KantarApp
{
	public partial class Form2 : Form
	{
		...
		private const int FrameLength = 11; // "0000000.000" toplam 11 karakter
		...
		private void SerialPort1_DataReceived(object sender, System.IO.Ports.SerialDataReceivedEventArgs e)
		{
			try
			{
				...
				lock (_rxLock)
				{
					...
					while (_incomingBuffer.Length >= FrameLength)
					{
						string frame = _incomingBuffer.Substring(0, FrameLength);

						_incomingBuffer = _incomingBuffer.Substring(FrameLength);

						this.BeginInvoke(new Action(() => ProcessKantarValue(frame)));
					}
				}
			}
			catch (Exception ex)
		...
		private void ProcessKantarValue(string rawData)
		{
			try
			{
				if (rawData.IndexOf('.') != 7)
				{
					return;
				}
		...
		private void ProcessSimpleLine(string rawData)
		{
			// Kantardan gelen 0000000.000000 gibi veriyi temizle
			string cleaned = rawData.Trim().Replace(',', '.');

			// Sayısal değer çekme denemesi
			if (decimal.TryParse(cleaned, NumberStyles.Any, CultureInfo.InvariantCulture, out decimal value))
			{
				UpdateQtyTextbox(value);
				//TryQueueSend(value);
